...
def process_thoughts():
    ...
    try:
        ...
        try:
            ...
            structured_data = {"raw_response": structured_json_str, "title": "Failed to Decode LLM Response", "classification": {}}


        # Render HTML from template
        logger.info("Rendering HTML pages.")
        renderer = HTMLRenderer(structured_data, settings.STATIC_DIR, settings.TEMPLATES_DIR)
        renderer.render()
        logger.info(f"Successfully generated static HTML pages in {settings.STATIC_DIR}")
